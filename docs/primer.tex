...
\begin{figure}
 ...
 The structure of the circuit is largely the same, except the original trigger register acts on a set of `indicator' qubits representative of each memory cell (highlighted in red).
 These indicator qubits then control coupling of the memory contents to the set of output bits.}
...
For the remainder of the query circuit, the $2^n$ Toffolis of the query have become $2^n$ CNOTs (from the trigger qubits to the indicator qubits of the cells), followed by $k 2^n$ sequential Toffolis. 
